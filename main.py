...
@app.route('/app')
def dashboard():
    """PWA dashboard - fonat 'Add to Home Screen' karता येईl asa ॲप-sarkha page."""
...
def api_status():
    """Dashboard cha JSON data source - pratyek symbol cha live status ani aajcha total P&L."""
    today_str = datetime.now(IST).strftime('%Y-%m-%d')
    symbols_data = []
    for symbol_key, cfg in SYMBOLS.items():
        ...
        symbols_data.append({
            ...
            "dir": s["last_status_dir"],
            "rsi": s["last_rsi"],
            "adx": s["last_adx"],
            "current_trade": s["current_trade"],
            "entry_price": s["entry_price"] if s["current_trade"] else None,
            "stop_loss": round(s["stop_loss"], 2) if s["current_trade"] else None,
        })
    total_pnl = get_today_total_pnl(today_str)
    return jsonify({
